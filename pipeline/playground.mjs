...
async function addDirectoryToFilesRecord(files, dir) {
  const exampleFilePaths = await (0, import_fast_glob.glob)("**/*", {
    cwd: dir,
    onlyFiles: true
  });
...
async function findAllConfigs(dir) {
  const configs = {};
  const paths = await (0, import_fast_glob.glob)("**/config.json", {
    cwd: dir,
    onlyFiles: true
  });
...
async function generatePlaygroundFiles(playgroundDir, commonDir, outputDir) {
  ...
  const commonPlaygroundDir = join3(playgroundDir, "common");
  ...
  for (const [path, config] of Object.entries(configs)) {
    ...
    const configDir = join3(playgroundDir, path);
    await addDirectoryToFilesRecord(itemFiles, configDir);
    mkdirSync(join3(outputDir, path), { recursive: true });
    writeFileSync(join3(outputDir, path, "metadata.json"), JSON.stringify(await generateMetadata(configDir, config, itemFiles)));
    writeFileSync(join3(outputDir, path, "source-code.json"), JSON.stringify(await generateSourceCode(config, itemFiles)));
  }
  writeFileSync(join3(outputDir, "routes.json"), JSON.stringify(await generatePlaygroundRoutes(configs)));
}
(async () => {
