...
// bazel-out/k8-fastbuild-ST-70f2edae98f4/bin/docs/markdown/tutorial/playground.js
...
async function generatePlaygroundRoutes(configs) {
  const templates = Object.entries(configs).map(([path2, config]) => ({
    path: `playground/${path2}`,
    label: config.title
  }));
  return {
    templates,
...
async function generatePlaygroundFiles(playgroundDir, commonDir, outputDir) {
  ...
  for (const [path2, config] of Object.entries(configs)) {
    const itemFiles = { ...files };
    await addDirectoryToFilesRecord(itemFiles, join2(playgroundDir, path2));
