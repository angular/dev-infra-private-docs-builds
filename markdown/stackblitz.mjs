...
// bazel-out/k8-fastbuild/bin/docs/markdown/examples/stackblitz/index.js
import { writeFileSync } from "fs";

// bazel-out/k8-fastbuild/bin/docs/markdown/examples/stackblitz/builder.js
import { join as join2 } from "path";
import { readFile } from "fs/promises";

// bazel-out/k8-fastbuild/bin/docs/markdown/examples/stackblitz/file-system.js
...
async function removeFolder(path2) {
  if (existsSync(path2)) {
    await rm(path2, { recursive: true });
  }
}
...
// node_modules/glob/node_modules/lru-cache/dist/mjs/index.js
...
import { lstat, readdir as readdir2, readlink, realpath } from "fs/promises";

// node_modules/glob/node_modules/minipass/dist/mjs/index.js
...
__export(block_c_exports, {
  ...
  plasterMatcher: () => plasterMatcher,
  regionEndMatcher: () => regionEndMatcher,
  regionStartMatcher: () => regionStartMatcher
});
var regionStartMatcher = /^\s*\/\*\s*#docregion\s*(.*)\s*\*\/\s*$/;
var regionEndMatcher = /^\s*\/\*\s*#enddocregion\s*(.*)\s*\*\/\s*$/;
...
__export(html_exports, {
  ...
  plasterMatcher: () => plasterMatcher2,
  regionEndMatcher: () => regionEndMatcher2,
  regionStartMatcher: () => regionStartMatcher2
});
var regionStartMatcher2 = /^\s*<!--\s*#docregion\s*(.*?)\s*(?:-->)?\s*$/;
var regionEndMatcher2 = /^\s*<!--\s*#enddocregion\s*(.*?)\s*-->\s*$/;
...
__export(inline_c_exports, {
  ...
  plasterMatcher: () => plasterMatcher3,
  regionEndMatcher: () => regionEndMatcher3,
  regionStartMatcher: () => regionStartMatcher3
});
var regionStartMatcher3 = /^\s*\/\/\s*#docregion\s*(.*)\s*$/;
var regionEndMatcher3 = /^\s*\/\/\s*#enddocregion\s*(.*)\s*$/;
...
__export(inline_c_only_exports, {
  ...
  plasterMatcher: () => plasterMatcher4,
  regionEndMatcher: () => regionEndMatcher4,
  regionStartMatcher: () => regionStartMatcher4
});
var regionStartMatcher4 = /^\s*\/\/\s*#docregion\s*(.*)\s*$/;
var regionEndMatcher4 = /^\s*\/\/\s*#enddocregion\s*(.*)\s*$/;
...
__export(inline_hash_exports, {
  ...
  plasterMatcher: () => plasterMatcher5,
  regionEndMatcher: () => regionEndMatcher5,
  regionStartMatcher: () => regionStartMatcher5
});
var regionStartMatcher5 = /^\s*#\s*#docregion\s*(.*)\s*$/;
var regionEndMatcher5 = /^\s*#\s*#enddocregion\s*(.*)\s*$/;
...
var REGION_MATCHERS = {
  ts: inline_c_exports,
  js: inline_c_exports,
  mjs: inline_c_only_exports,
  es6: inline_c_exports,
  html: html_exports,
  svg: html_exports,
  css: block_c_exports,
  conf: inline_hash_exports,
  yaml: inline_hash_exports,
  yml: inline_hash_exports,
  sh: inline_hash_exports,
  jade: inline_c_only_exports,
  pug: inline_c_only_exports,
  json: inline_c_only_exports,
  "json.annotated": inline_c_only_exports
};
var regionParser = (contents, fileType) => {
...
// bazel-out/k8-fastbuild/bin/docs/markdown/examples/stackblitz/copyright.js
var PAD = "\n\n";
var COPYRIGHT = `
@license
...
`;
var CSS_TS_COPYRIGHT = `/*${COPYRIGHT}*/${PAD}`;
var HTML_COPYRIGHT = `<!--${COPYRIGHT}-->${PAD}`;
function appendCopyrightToFile(filename, content) {
...
// bazel-out/k8-fastbuild/bin/docs/markdown/examples/stackblitz/defaults.js
...
var EXAMPLE_CONFIG_FILENAME = "example-config.json";
var STACKBLITZ_CONFIG_FILENAME = "stackblitz.json";
var EXCLUDE_FILES_FOR_STACKBLITZ = [
...
async function generateStackblitzExample(exampleDir2, temporaryExampleDir, stackblitzTemplateDir) {
  ...
  await copyFolder(exampleDir2, temporaryExampleDir);
...
function getPrimaryFile(primaryFilePath, exampleFilePaths) {
  if (primaryFilePath) {
    ...
    const primaryFile = defaultPrimaryFilePaths.find((path2) => exampleFilePaths.some((filePath) => filePath === path2));
    if (!primaryFile) {
...
async function createPostData(exampleDir2, config, exampleFilePaths) {
  const postData = {};
  for (const filePath of exampleFilePaths) {
    if (EXCLUDE_FILES_FOR_STACKBLITZ.some((excludedFile) => filePath.endsWith(excludedFile))) {
      continue;
    }
    let content = await readFile(join2(exampleDir2, filePath), "utf-8");
...
function extractRegions(path2, contents) {
  const fileType = path2?.split(".").pop();
 ...
 * found in the LICENSE file at https://angular.dev/license
 */
//# sourceMappingURL=stackblitz.mjs.map
