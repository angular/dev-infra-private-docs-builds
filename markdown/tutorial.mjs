...
var jpgSignature = [255, 216, 255];
var gifSignature = [71, 73, 70];
var pngSignature = [137, 80, 78, 71, 13, 10, 26, 10];
var SIGNATURES = [jpgSignature, gifSignature, pngSignature];
function getFileContents(path2) {
  const fileBuffer = readFileSync(path2);
...
async function addDirectoryToFilesRecord(files, dir) {
  const exampleFilePaths = await glob("**/*", {
    cwd: dir,
    nodir: true
  });
  for (let path2 of exampleFilePaths) {
    files[path2] = await getFileContents(join(dir, path2));
  }
}
async function findAllConfigs(dir) {
  const configs = {};
  ...
  for (const path2 of paths) {
    const content = await getFileContents(join(dir, path2));
    configs[dirname(path2)] = JSON.parse(content);
  }
  return configs;
}
...
function buildFileSystemTree(fileSystemTree, fileDirectories, filename, fileContents) {
  if (fileDirectories.length === 1) {
    ...
    fileSystemTree[directory] = {
      ...fileSystemTree[directory],
      directory: {
        ...fileSystemTree[directory] ? fileSystemTree[directory].directory : void 0,
        [filename]: fileNode
      }
    };
    return;
...
async function generateTutorialRoutes(tutorialName, introConfig, stepConfigs) {
  const children = Object.entries(stepConfigs).sort(([pathA], [pathB]) => Number(pathA.split("-")[0]) > Number(pathB.split("-")[0]) ? 1 : -1).map(([path2, config], idx) => {
    path2 = path2.replace(/^\d+\-/, "");
    return {
      label: config.title,
      path: `tutorials/${tutorialName}/${path2}`,
      contentPath: `tutorials/${tutorialName}/steps/${path2}/README`,
      tutorialData: {
        title: config.title,
        type: config.type,
        step: idx + 1
...
async function generateTutorialFiles(tutorialDir, commonDir, outputDir) {
  ...
  for (const [path2, config] of Object.entries(stepConfigs)) {
    const itemFiles = { ...files };
    await addDirectoryToFilesRecord(itemFiles, join2(tutorialDir, "steps", path2));
    mkdirSync(join2(outputDir, path2), { recursive: true });
    writeFileSync(join2(outputDir, path2, "metadata.json"), JSON.stringify(await generateMetadata(config, itemFiles)));
    writeFileSync(join2(outputDir, path2, "source-code.json"), JSON.stringify(await generateSourceCode(config, itemFiles)));
  }
